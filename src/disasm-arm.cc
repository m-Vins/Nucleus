...
is_cs_ret_ins(cs_insn *ins)
{
  ...
  /* bx lr */
  if (ins->id == ARM_INS_BX && ins->detail->arm.op_count == 1 && ins->detail->arm.operands[0].type == ARM_OP_REG && ins->detail->arm.operands[0].reg == ARM_REG_LR)
  {
    return 1;
  }
  ...
  /* mov pc, lr */
  if (ins->id == ARM_INS_MOV && ins->detail->arm.operands[0].type == ARM_OP_REG && ins->detail->arm.operands[0].reg == ARM_REG_PC && ins->detail->arm.operands[1].type == ARM_OP_REG && ins->detail->arm.operands[1].reg == ARM_REG_LR)
  {
    return 1;
  }
...
is_cs_unconditional_jmp_ins(cs_insn *ins)
{
  ...
  /* ldrls pc, {...} */
  if (ins->id == ARM_INS_LDR && ins->detail->arm.operands[0].type == ARM_OP_REG && ins->detail->arm.operands[0].reg == ARM_REG_PC)
  {
    return 1;
  }
