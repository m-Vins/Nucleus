...
void CFG::print_functions(FILE *out)
{
...
void CFG::print_function_summaries(FILE *out)
{
  for (auto &f : this->functions)
  {
    f.print_summary(out);
...
void CFG::mark_addrtaken(uint64_t addr)
{
  BB *cc;
...
void CFG::analyze_addrtaken_ppc()
{
  BB *bb;
  ...
  for (auto &kv : this->start2bb)
  {
    ...
    for (auto &ins : bb->insns)
    {
      ...
      else if (ins.operands[0].type == Operand::OP_TYPE_REG && ins.operands[0].ppc_value.reg >= PPC_REG_R0 && ins.operands[0].ppc_value.reg <= PPC_REG_R31)
      {
        int64_t dst = ins.operands[0].ppc_value.reg - PPC_REG_R0;
        registers[dst] = -1;
      }
...
void CFG::analyze_addrtaken_x86()
{
  BB *bb;
  ...
  for (auto &kv : this->start2bb)
  {
    ...
    for (auto &ins : bb->insns)
    {
      ...
      if (((op_dst->type == Operand::OP_TYPE_REG) || (op_dst->type == Operand::OP_TYPE_MEM)) && (op_src->type == Operand::OP_TYPE_IMM))
      {
...
void CFG::mark_jmptab_as_data(uint64_t start, uint64_t end)
{
  uint64_t addr;
  BB *bb, *cc;
...
void CFG::find_switches_aarch64()
{
  BB *bb, *cc;
  Edge *conflict_edge;
  Section *target_sec;
    ...
    if (bb->insns.back().edge_type() == Edge::EDGE_TYPE_JMP_INDIRECT)
    {
      ...
      for (auto &ins : bb->insns)
      {
        ...
        if (scale == 0)
        {
          ...
          else if (ins.id == ARM64_INS_LDR && ins.operands[0].aarch64_value.reg >= ARM64_REG_W0 && ins.operands[0].aarch64_value.reg <= ARM64_REG_W28)
          {
            scale = 4;
          }
          else if (ins.id == ARM64_INS_LDR && ins.operands[0].aarch64_value.reg >= ARM64_REG_X0 && ins.operands[0].aarch64_value.reg <= ARM64_REG_X28)
          {
            scale = 8;
          }
        }
        ...
        else if (ins.id == ARM64_INS_ADD && ins.operands[1].type == Operand::OP_TYPE_REG && ins.operands[2].type == Operand::OP_TYPE_IMM)
        {
          int64_t dst = ins.operands[0].aarch64_value.reg - ARM64_REG_X0;
        ...
        else if (ins.operands[0].type == Operand::OP_TYPE_REG && ins.operands[0].aarch64_value.reg >= ARM64_REG_X0 && ins.operands[0].aarch64_value.reg <= ARM64_REG_X28)
        {
          int64_t dst = ins.operands[0].aarch64_value.reg - ARM64_REG_X0;
          registers[dst] = -1;
        }
  ...
  for (auto &kv : this->start2bb)
  {
    ...
    if (bb->insns.back().edge_type() == Edge::EDGE_TYPE_JMP_INDIRECT)
    {
      ...
      for (auto rit = bb->insns.rbegin(); rit != bb->insns.rend(); ++rit)
      {
        ...
        else if (ins.id == ARM_INS_LDR && ins.operands[0].arm_value.reg == ARM_REG_PC && ins.operands[1].arm_value.reg == ARM_REG_PC)
        {
  ...
  for (auto &kv : this->start2bb)
  {
    ...
    if (bb->insns.back().edge_type() == Edge::EDGE_TYPE_JMP_INDIRECT)
    {
      ...
      for (auto &ins : bb->insns)
      {
        ...
        else if (ins.operands[0].type == Operand::OP_TYPE_REG && ins.operands[0].mips_value.reg >= MIPS_REG_0 && ins.operands[0].mips_value.reg <= MIPS_REG_31)
        {
          int64_t dst = ins.operands[0].mips_value.reg - MIPS_REG_0;
          registers[dst] = -1;
        }
...
void CFG::expand_function(Function *f, BB *bb)
{
  if (!bb)
  {
    bb = f->BBs.front();
  ...
  if (!bb->is_called())
  {
    ...
    {
      if ((e.type == Edge::EDGE_TYPE_CALL) || (e.type == Edge::EDGE_TYPE_CALL_INDIRECT) || (e.type == Edge::EDGE_TYPE_RET))
      {
        continue;
      }
...
void CFG::verify_padding()
{
  ...
  for (auto &kv : this->start2bb)
  {
    ...
    if (bb->padding && !bb->ancestors.empty())
    {
      ...
      for (auto &e : bb->ancestors)
      {
        if ((e.type == Edge::EDGE_TYPE_FALLTHROUGH) && (e.src->insns.back().flags & Instruction::INS_FLAG_CALL))
        {
...
BB *CFG::get_bb(uint64_t addr, unsigned *offset)
{
  BB *bb;
...
void CFG::link_bbs(Edge::EdgeType type, BB *bb, uint64_t target, uint64_t jmptab)
{
  BB *cc;
  bool is_switch;
  unsigned offset;
...
void CFG::unlink_edge(BB *bb, BB *cc)
{
  std::list<Edge>::iterator f;
...
int CFG::make_cfg(Binary *bin, std::list<DisasmSection> *disasm)
{
  uint64_t addr;
  unsigned flags;
